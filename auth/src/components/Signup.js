...
const Signup = ({
  ...
  const signup = async () => {
    ...
    try {
      ...
      setSuccess(`User ${fullName} registered! Please go to Login page to access!`);
      setUsername('')
      setPassword('')
      setFullName('')
    } catch (e) {
      ...
      <Grid item xs={11} md={3}>
        <Paper className={classes.paper}>
          ...
          <Grid container direction={'row'} spacing={2}>
            <Grid item xs={12}>
              <TextField fullWidth label="Full name" variant="filled" onChange={e => setFullName(e.target.value)} />
            </Grid>
            <Grid item xs={12}>
            ...
            <Grid item xs={12}>
              <Button variant="contained" color="primary" disableElevation fullWidth onClick={signup}>
                Signup
              </Button>
            </Grid>
            <Grid item xs={12}>
